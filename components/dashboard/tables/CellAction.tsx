...
interface CellActionQuestionProps {
  data: Question;
}

interface CellActionGamePlayerProps {
  data: GamePlayer;
}
...
export const CellActionGamePlayer: React.FC<CellActionGamePlayerProps> = ({
  data,
}) => {
  const [loading, setLoading] = useState(false);
  const [open, setOpen] = useState(false);
      ...
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem
            onClick={() => router.push(`/dashboard/games/${data.id}`)}
          >
            <Eye className="mr-2 h-4 w-4" /> Lihat Jawaban
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => {}}>
            <Edit className="mr-2 h-4 w-4" /> Tambah Jawaban
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => setOpen(true)}>
            <Trash className="mr-2 h-4 w-4" /> Hapus
          </DropdownMenuItem>
...
export const CellActionQuestion: React.FC<CellActionQuestionProps> = ({
  data,
}) => {
  const [loading, setLoading] = useState(false);
  const [open, setOpen] = useState(false);
